...
for i = 1:size(Points, 1)
    for j = i + 1:size(Points, 1)
        ...
        if d <= 2
            A(i, j) = 0.75 * d * (d - 1) + 1;  % Adjusted weight
            A(j, i) = 0.75 * d * (d - 1) + 1;  % Adjusted weight
        end
    end
end
...
% Replace graphallshortestpaths with distances and correct the output
G = graph(SA);  % Create the graph
dist = distances(G);  % Compute all-pairs shortest paths (distances)
