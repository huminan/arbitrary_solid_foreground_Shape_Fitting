function displayResults(shapeMetrics, ellipseMetrics, contourMetrics)
    % 显示评估结果
    % 参数:
    ...
    fprintf(' 形状指标 \n');
    fprintf('%s\n', repmat('-', 1, 20));
    fprintf('形状覆盖率 (αE)    : %.4f (%.2f%%)\n', shapeMetrics.coverageRate, shapeMetrics.coverageRate*100);
    fprintf('AIC                : ');
    for id = 1:length(shapeMetrics.AIC)
        fprintf('%.4f, ', shapeMetrics.AIC(id));
    end
    fprintf('\n');
    fprintf('真阳性 (TP)        : %d\n', shapeMetrics.TP);
    fprintf('假阳性 (FP)        : %d\n', shapeMetrics.FP);
    fprintf('假阴性 (FN)        : %d\n', shapeMetrics.FN);
    fprintf('Dice假阴性 (FN)    : %.4f\n', shapeMetrics.DiceFN);
    fprintf('Dice假阳性 (FP)    : %.4f\n', shapeMetrics.DiceFP);
    fprintf('Precision          : %.4f\n', shapeMetrics.Precision);
    fprintf('Recall             : %.4f\n', shapeMetrics.Recall);
    fprintf('f1-score           : %.4f\n', shapeMetrics.f1);
