...
function [] = displayCoverage(GTBinary, GT, res, save)
...
overlayImg(:,:,2) = double(res.segFilledBinary) * 0.8;       % 绿色通道用于拟合结果
overlayImg(:,:,3) = double(GTBinary & res.segFilledBinary) * 0.9;  % 蓝色通道用于交集
...
if ~isempty(save)
    ...
    exportgraphics(gcf, sprintf("%s%s_coverage.tiff", save, res.name), 'Resolution', 300);
end
